#!/usr/bin/env python3
"""HTML template for V4L2 camera controls interface."""

HTML_CONTROLS = """<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body>
  ...
  <main>
    <section class="panel panel--preview">
      ...
      <div class="note">Changes are persisted by the control service on apply.</div>
    </section>
    <section class="panel">
  ...
  <script>
    ...
    const storageKey = (key) => `v4l2ctrl-${key}`;
    ...
    async function rpcCall(method, params) {
      ...
      const response = await fetch('/control/rpc', {
        method: 'POST',
    ...
    function updatePreview() {
      ...
      if (mode === 'webrtc') {
        preview.innerHTML = '<iframe src="/webrtc"></iframe>';
      } else if (mode === 'mjpg') {
        preview.innerHTML = '<img src="/stream.mjpg" alt="MJPG stream" />';
      } else {
        const url = appendCacheBuster('/snapshot.jpg');
        preview.innerHTML = `<img src="${url}" alt="Snapshot" />`;
      }
    ...
    async function applyChanges() {
      ...
      try {
        const data = await rpcCall('set', {controls: payload});
        if (!data.ok) {
          throw new Error(data.error || 'Failed to apply controls');
        }
        logStatus(`Applied: ${JSON.stringify(data.applied, null, 2)}`.trim());

        // Re-fetch all controls to get updated values
